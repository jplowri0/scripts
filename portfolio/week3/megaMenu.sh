...
read -sp "$(tput setaf 1) Enter Password:$(tput sgr 0) " passCheck #Colour set by bounding the "Enter Password". See REF F
...
if [ "$passphrase" = "-: OK" ]; then
    echo "$(tput setaf 2)Access Granted$(tput sgr 0)" #Colour set by bounding the "Access Granted" See REF F
    for ((i=0; ;++i)); do #This for loop, loops infitely until the user enters option 8 and exits the program. REF G
        echo "$(tput setaf 4) Choose an Option:$(tput sgr 0)" #Colour set by bounding the "Choose an Options" See REF F
        echo "$(tput setaf 6)1. Create a folder" #Open the colour setting for the menu options. See REF F
        echo "2. Copy a folder"
        echo "3. Set a Passord"
        echo "4. Calculator"
        echo "5. Create Week Folders"
        echo "6. Check Filename"
        echo "7. Download a File$(tput sgr 0)" #Closing the colour setting for the menu options. See REF F
        echo "8. Exit"
        read option;
