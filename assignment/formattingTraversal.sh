...
for ((i=1;i<2;++i)); do 
    inputData="$( cat scrapedData/Traversal/watchOStraversal$i.html )"
    ...
    echo "$inputData" | sed -n '/<td nowrap/ { 
        ...
        p
        }' > workingData/traversalCVECode/traversalCVEcode$i.txt
    ...
    echo "$inputData" | sed -n '/					<td><div class=/ { 
        ...
        s/<\/div><\/td>//
        p
        }' > workingData/traversalRatingCode/traversalRatingCode$i.txt
    ...
    echo "$inputData" | sed -n '/					<td class=\"cvesummarylong\" colspan=\"20\">/ { 
        ...
        s/					<\/td>//
        p	
        }' > workingData/traversalDesCode/traversalDesCode$i.txt
