...
}' workingData/scrapedInformationTSV.txt
...
average=$(awk '{ total += $1; count++ } END { print total/count }' workingData/informationRatingCode/informationRatingCodeAll.txt)
echo "The average threat rating is:  $average"
count=$(wc -l <workingData/informationRatingCode/informationRatingCodeAll.txt)
echo "There are $count Gained Information threats."
...
for ((i=0; ;++i)); do 
    ...
    if [ $choice == Y ]; then
        ...
        if [ -d "$destination" ]; then
            cp workingData/scrapedInformation.csv $destination
