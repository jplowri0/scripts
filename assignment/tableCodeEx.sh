...
}' workingData/scrapedCodeExTSV.txt
...
average=$(awk '{ total += $1; count++ } END { print total/count }' workingData/codeExRatingCode/codeExRatingCodeAll.txt)
echo "The average threat rating is:  $average"
count=$(wc -l <workingData/codeExRatingCode/codeExRatingCodeAll.txt)
echo "There are $count Execute Code Threats."
...
for ((i=0; ;++i)); do 
    ...
    if [ $choice == Y ]; then
        ...
        if [ -d "$destination" ]; then
            cp workingData/scrapedCodeEx.csv $destination
