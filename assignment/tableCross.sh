...
}' workingData/scrapedCrossTSV.txt
...
average=$(awk '{ total += $1; count++ } END { print total/count }' workingData/crossRatingCode/crossRatingCode1.txt)
echo The average threat rating is:  $average
count=$(wc -l <workingData/crossRatingCode/crossRatingCode1.txt)
echo There are $count Cross Site Scripting threats.
...
for ((i=0; ;++i)); do 
    ...
    if [ $choice == Y ]; then
        ...
        if [ -d "$destination" ]; then
            cp workingData/scrapedCross.csv $destination
