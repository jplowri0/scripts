...
for ((i=1;i<6;++i)); do 
    inputData="$( cat scrapedData/Memory/watchOSmemory$i.html )"
    ...
    echo "$inputData" | sed -n '/<td nowrap/ { 
        ...
        p
        }' > workingData/memoryCVECode/memoryCVEcode$i.txt
    ...
    echo "$inputData" | sed -n '/					<td><div class=/ { 
        ...
        s/<\/div><\/td>//
        p
        }' > workingData/memoryRatingCode/memoryRatingCode$i.txt
    ...
    echo "$inputData" | sed -n '/					<td class=\"cvesummarylong\" colspan=\"20\">/ { 
        ...
        s/					<\/td>//
        p	
        }' > workingData/memoryDesCode/memoryDesCode$i.txt
