...
CVEcodeTxt = open("crossCVECode/crossCVEcode1.txt", "r")
...
ratingCodeTxt = open("crossRatingCode/crossRatingCode1.txt", "r")
...
desCodeTxt = open("crossDesCode/crossDesCode1.txt", "r")
...
with open('scrapedCross.csv', "w") as f:
    writer = csv.writer(f)
    for row in rows:
        writer.writerow(row)

with open('scrapedCross.csv', 'r') as inputCSV, open('scrapedCrossTSV.txt', 'w') as outputTSV:
