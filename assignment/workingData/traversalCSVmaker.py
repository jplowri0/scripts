...
CVEcodeTxt = open("traversalCVECode/traversalCVEcode1.txt", "r")
...
ratingCodeTxt = open("traversalRatingCode/traversalRatingCode1.txt", "r")
...
desCodeTxt = open("traversalDesCode/traversalDesCode1.txt", "r")
...
with open('scrapedTraversal.csv', "w") as f:
    writer = csv.writer(f)
    for row in rows:
        writer.writerow(row)

with open('scrapedTraversal.csv', 'r') as inputCSV, open('scrapedTraversalTSV.txt', 'w') as outputTSV:
