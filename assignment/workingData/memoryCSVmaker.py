...
CVEcodeTxt = open("memoryCVECode/memoryCVECodeAll.txt", "r")
...
ratingCodeTxt = open("memoryRatingCode/memoryRatingCodeAll.txt", "r")
...
desCodeTxt = open("memoryDesCode/memoryDesCodeAll.txt", "r")
...
with open('scrapedMemory.csv', "w") as f:
    writer = csv.writer(f)
    for row in rows:
        writer.writerow(row)

with open('scrapedMemory.csv', 'r') as inputCSV, open('scrapedMemoryTSV.txt', 'w') as outputTSV:
