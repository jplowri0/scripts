...
CVEcodeTxt = open("bypassCVECode/bypassCVEcode1.txt", "r")
...
ratingCodeTxt = open("bypassRatingCode/bypassRatingCode1.txt", "r")
...
desCodeTxt = open("bypassDesCode/bypassDesCode1.txt", "r")
...
with open('scrapedBypass.csv', "w") as f:
    writer = csv.writer(f)
    for row in rows:
        writer.writerow(row)

with open('scrapedBypass.csv', 'r') as inputCSV, open('scrapedBypassTSV.txt', 'w') as outputTSV:
