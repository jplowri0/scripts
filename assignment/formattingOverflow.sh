...
for ((i=1;i<7;++i)); do 
    inputData="$( cat scrapedData/Overflow/watchOSoverflow$i.html )"
    ...
    echo "$inputData" | sed -n '/<td nowrap/ { 
        ...
        p
        }' > workingData/overflowCVECode/overflowCVEcode$i.txt
    ...
    echo "$inputData" | sed -n '/					<td><div class=/ { 
        ...
        s/<\/div><\/td>//
        p
        }' > workingData/overflowRatingCode/overflowRatingCode$i.txt
    ...
    echo "$inputData" | sed -n '/					<td class=\"cvesummarylong\" colspan=\"20\">/ { 
        ...
        s/					<\/td>//
        p	
        }' > workingData/overflowDesCode/overflowDesCode$i.txt
