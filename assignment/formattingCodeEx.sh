...
for ((i=1;i<6;++i)); do 
    inputData="$( cat scrapedData/CodeEx/watchOScodeEx$i.html )"
    ...
    echo "$inputData" | sed -n '/<td nowrap/ { 
        ...
        p
        }' > workingData/codeExCveCode/codeExCVEcode$i.txt
    ...
    echo "$inputData" | sed -n '/					<td><div class=/ { 
        ...
        s/<\/div><\/td>//
        p
        }' > workingData/codeExRatingCode/codeExRatingCode$i.txt
    ...
    echo "$inputData" | sed -n '/					<td class=\"cvesummarylong\" colspan=\"20\">/ { 
        ...
        s/					<\/td>//
        p	
        }' > workingData/codeExDesCode/codeExDesCode$i.txt
