...
}' workingData/scrapedPrivelegesTSV.txt
...
average=$(awk '{ total += $1; count++ } END { print total/count }' workingData/privelegesRatingCode/privelegesRatingCode1.txt)
echo The average threat rating is:  $average
count=$(wc -l <workingData/privelegesRatingCode/privelegesRatingCode1.txt)
echo There are $count Gain Priveleges threats.
...
for ((i=0; ;++i)); do 
    ...
    if [ $choice == Y ]; then
        ...
        if [ -d "$destination" ]; then
            cp workingData/scrapedPriveleges.csv $destination
