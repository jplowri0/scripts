...
}' workingData/scrapedMemoryTSV.txt
...
average=$(awk '{ total += $1; count++ } END { print total/count }' workingData/memoryRatingCode/memoryRatingCodeAll.txt)
echo The average threat rating is:  $average
count=$(wc -l <workingData/memoryRatingCode/memoryRatingCodeAll.txt)
echo There are $count Memory Corruption threats.
...
for ((i=0; ;++i)); do 
    ...
    if [ $choice == Y ]; then
        ...
        if [ -d "$destination" ]; then
            cp workingData/scrapedMemory.csv $destination
