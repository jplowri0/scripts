...
for ((i=1;i<2;++i)); do 
    inputData="$( cat scrapedData/Priveleges/watchOSpriveleges$i.html )"
    ...
    echo "$inputData" | sed -n '/<td nowrap/ { 
        ...
        p
        }' > workingData/privelegesCVECode/privelegesCVEcode$i.txt
    ...
    echo "$inputData" | sed -n '/					<td><div class=/ { 
        ...
        s/<\/div><\/td>//
        p
        }' > workingData/privelegesRatingCode/privelegesRatingCode$i.txt
    ...
    echo "$inputData" | sed -n '/					<td class=\"cvesummarylong\" colspan=\"20\">/ { 
        ...
        s/					<\/td>//
        p	
        }' > workingData/privelegesDesCode/privelegesDesCode$i.txt
