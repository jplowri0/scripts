...
for ((i=1;i<2;++i)); do 
    inputData="$( cat scrapedData/Injection/watchOSinjection$i.html )"
    ...
    echo "$inputData" | sed -n '/<td nowrap/ { 
        ...
        p
        }' > workingData/injectionCVECode/injectionCVEcode$i.txt
    ...
    echo "$inputData" | sed -n '/					<td><div class=/ { 
        ...
        s/<\/div><\/td>//
        p
        }' > workingData/injectionRatingCode/injeectionRatingCode$i.txt
    ...
    echo "$inputData" | sed -n '/					<td class=\"cvesummarylong\" colspan=\"20\">/ { 
        ...
        s/					<\/td>//
        p	
        }' > workingData/injectionDesCode/injectionDesCode$i.txt
