...
}' workingData/scrapedInjectionTSV.txt
...
average=$(awk '{ total += $1; count++ } END { print total/count }' workingData/injectionRatingCode/injeectionRatingCode1.txt)
echo The average threat rating is:  $average
count=$(wc -l <workingData/injectionRatingCode/injeectionRatingCode1.txt)
echo There are $count SQL Injection threats.
...
for ((i=0; ;++i)); do 
    ...
    if [ $choice == Y ]; then
        ...
        if [ -d "$destination" ]; then
            cp workingData/scrapedInjection.csv $destination
